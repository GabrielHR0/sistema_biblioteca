class LoansController < ApplicationController
  ...
  before_action :require_staff, only: %i[index create update destroy renew return active_by_copy]
  before_action :set_loan, only: %i[show update destroy renew return]
  ...
  def index
    ...
    if params[:client_id].present?
      @loans = @loans.where(client_id: params[:client_id])
    end
  ...
  # GET /loans/active_by_copy
  def active_by_copy
    copy_id = params[:copy_id]
    
    if copy_id.blank?
      return render json: { error: "copy_id é obrigatório" }, status: :unprocessable_entity
    end

    # Busca empréstimos ativos para uma cópia específica
    @loans = Loan.includes(:client, :copy)
                .where(copy_id: copy_id, status: "ongoing")
  ...
  def return
    if @loan.status == "returned"
      return render json: { error: "Este empréstimo já foi devolvido anteriormente." }, status: :unprocessable_entity
    end

    puts "#{@loan.inspect}"

    @loan.status = "returned"
    @loan.return_date = Time.zone.today
    @loan.user_id = @current_user.id
    @loan.copy.update(status: "available")
